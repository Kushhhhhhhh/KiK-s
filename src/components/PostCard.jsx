...
const PostCard = ({post, user, deletePost, likePost}) => {                            const getComments = async () => {
    setReplyComments(0);
      ...
      <span className="text-blue cursor-pointer"
          ...
          <div className="py-2 px-8 mt-6">
            {comment?.replies?.length > 0 && (
                <p className="cursor-pointer text-base text-ascent-1"
                onClick={()=> setShowReply(
                    showReply === comment?.replies._id ? null : comment?.replies?._id
                )}
                >
